...
#if defined(SKINNED)
    #define iModelMatrix GetSkinMatrix(IN.iBlendWeights, IN.iBlendIndices);
#elif defined(INSTANCED)
    #define iModelMatrix IN.iModelInstance
#else
    #define iModelMatrix cModel
#endif

#ifdef BILLBOARD
    #define GetWorldPos(modelMatrix) GetBillboardPos(IN.iPos, IN.iSize, modelMatrix)
#else
    #define GetWorldPos(modelMatrix) mul(IN.iPos, modelMatrix)
#endif

#ifdef BILLBOARD
    #define GetWorldNormal(modelMatrix) GetBillboardNormal()
#else
    #define GetWorldNormal(modelMatrix) normalize(mul(IN.iNormal, (float3x3)modelMatrix))
#endif

#define GetWorldTangent(modelMatrix) normalize(mul(IN.iTangent.xyz, (float3x3)modelMatrix))
#endif
