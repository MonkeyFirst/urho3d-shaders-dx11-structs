	...
	OUT.oWorldPos = float4(worldPos, GetDepth(OUT.oPos));
		...
		OUT.oVertexLight = GetAmbient(GetZonePos(worldPos));
            ...
            for (int i = 0; i < NUMVERTEXLIGHTS; ++i)
				OUT.oVertexLight += GetVertexLightVolumetric(i, worldPos) * cVertexLights[i * 3].rgb;
        #endif
    #endif
...
PS_OUTPUT PS(PS_INPUT IN)
{
    ...
    // Get fog factor
		...
		float diff = GetDiffuseVolumetric(IN.iWorldPos.xyz);
			...
			lightColor = texCUBE(sLightCubeMap, IN.iCubeMaskVec).rgb * cLightColor.rgb;
        #else
