...
PS_OUTPUT PS(PS_INPUT IN)
{
    ...
    float depth = Sample2DLod0(DepthBuffer, IN.iScreenPos).r;
        ...
        #endif
                float4 albedoInput = Sample2DLod0(AlbedoBuffer, IN.iScreenPos);
    ...
    float depth = Sample2DProj(DepthBuffer, IN.iScreenPos).r;
        ...
        #endif
                float4 albedoInput = Sample2DProj(AlbedoBuffer, IN.iScreenPos);
    ...
    float diff = GetDiffuse(normal, worldPos, lightDir);
        ...
        OUT.oColor = diff * float4(lightColor * (albedoInput.rgb + spec * cLightColor.a * albedoInput.aaa), 0.0);
    #else
        OUT.oColor = diff * float4(lightColor * albedoInput.rgb, 0.0);
    #endif
    return OUT;
}
#endif